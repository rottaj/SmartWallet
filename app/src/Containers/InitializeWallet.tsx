import { useEffect, useContext } from 'react'
import { ethers } from 'ethers';
import {
    Box,
    Heading,
    Input,
    Button
} from "@chakra-ui/react"
import { handleWalletConnection } from '../utils/HandleWalletConnection';
import { WalletContext } from '../contexts';

const alchemy_url: any = process.env.REACT_APP_ALCHEMY_RPC;
const priv_key: any = process.env.REACT_APP_PRIV_KEY;



const InitializeWallet = () => {

    const { chrome, setProvider, setWallet}: any = useContext(WalletContext)

    useEffect(() => {

    }, [])

    const handleSubmit = async (e: any) => {

        e.preventDefault();
        // hash password save it to storage "passwordHash"

        const provider = new ethers.providers.JsonRpcProvider(alchemy_url)
        setProvider(provider)

        const newWallet = ethers.Wallet.createRandom();
        const wallet = new ethers.Wallet(newWallet.privateKey);
        //const wallet = await handleWalletConnection(priv_key, provider);
        setWallet(wallet)

        chrome.storage.sync.set({[`${e.target[0].value}`]: wallet}, function() {
            console.log("ACCOUNT CREATED", e.target[0].value, 'Account 1', Object.getOwnPropertyNames(wallet))
        })
    }

    return (
         <Box minH="100vh" h="100%" bgColor="#141114" textAlign="center" pt="40px">
            <Heading color="white" fontSize="30px">Create a new Smart Wallet</Heading>
            <form onSubmit={handleSubmit}>
                <Input placeholder="Choose a Password" px="100px" py="15px"></Input>
                <Input pt="15px" placeholder="Re-Type Password" px="100px" py="15px"></Input>
                <Button
                bgColor="#06d6a0"
                _hover={{ bgColor: "#95d5b2" }}
                color="black"
                mt="22px"
                p="8"
                type="submit"
                >
                    Create Wallet
                </Button>
            </form>
        </Box>       
    )
}

export default InitializeWallet;